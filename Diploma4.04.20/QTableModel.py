...
class pandasModel(QtCore.QAbstractTableModel):
    ...
    def data(self, index, role=QtCore.Qt.DisplayRole):
        ...
        if role == QtCore.Qt.EditRole:
            row = index.row()
            column = index.column()
            return self.__data.iloc[row, column]  # Определяем позицию элемента в массиве
            # Возвращяем значение из массива в отображение

        # Отображение данных в представлении ========================================
        if role == QtCore.Qt.DisplayRole:
            row = index.row()  # Определяем индекс элемента в строке
            column = index.column()  # Определяем индекс элемента в столбце
            return str(self.__data.iloc[row, column])
