...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):  # ---шаблон
        ...
        #self.tableView_1.setStyleSheet(tableView_1)  # Применение стилей для наших таблиц
        ...
        self.new_df_for_saving = pd.DataFrame()  # Датафрейм для хранения результата
        ...
        """Привязка чекбоксов"""
        self.checkBox_year.stateChanged.connect(self.checkBox_year_selected)
        self.checkBox_month.stateChanged.connect(self.checkBox_month_selected)
        self.checkBox_day.stateChanged.connect(self.checkBox_day_selected)
        self.checkBox_hour.stateChanged.connect(self.checkBox_hour_selected)
        self.checkBox_min.stateChanged.connect(self.checkBox_min_selected)
        self.checkBox_sec.stateChanged.connect(self.checkBox_sec_selected)
    ...
    def insert_dataframe1(self):
        ...
        if len(file_path) > 0:
            ...
            if 'xlsx' in file_path:
                ...
                # self.new_df1 = df1  # Записываем в new_df1 для дальнейшей работы с ним
                ...
                self.dataframe1model = pandasModel(self.new_df1)  # Как ургумент передаем новый датафрейм
    ...
    def insert_dataframe2(self):
        ...
        """ Метод для обработки чекбоксов """

    def checkBox_year_selected(self):

        if self.checkBox_year.isChecked() == True:
            self.tableView_1.setStyleSheet(tableView_1)  # Применение стиля при выборе годов
            self.tableView_2.setStyleSheet(tableView_2)

        if self.checkBox_year.isChecked() == False:

            self.tableView_1.setStyleSheet("all: unset; ")  # Отмена стиля
            self.tableView_2.setStyleSheet("all: unset; ")  # Отмена стиля

    def checkBox_month_selected(self):
        if self.checkBox_month.isChecked() == True:
            print("month is Checked !")
        if self.checkBox_month.isChecked() == False:
            print("month is unchecked !")
        else:
            return False

    def checkBox_day_selected(self):
        if self.checkBox_month.isChecked() == True:
            print("day is Checked !")
        if self.checkBox_month.isChecked() == False:
            print("day is unchecked !")
        else:
            return False


    def checkBox_hour_selected(self):
        if self.checkBox_hour.isChecked() == True:
            print("hour is Checked !")
        if self.checkBox_hour.isChecked() == False:
            print("hour is unchecked !")
        else:
            return False

    def checkBox_min_selected(self):
        if self.checkBox_min.isChecked() == True:
            print("min is Checked !")
        if self.checkBox_min.isChecked() == False:
            print("min is unchecked !")
        else:
            return False

    def checkBox_sec_selected(self):
        if self.checkBox_sec.isChecked() == True:
            print("sec is Checked !")
        if self.checkBox_sec.isChecked() == False:
            print("sec is unchecked !")
        else:
            return False
        ...
        if self.new_df1.empty or self.new_df2.empty:  # Проверка на имеющиеяся данные
            ...
            if self.checkBox_year.isChecked() == True:
                ...
                try:
                    ...
                    self.new_df_for_saving['depth_difference'] = self.new_df_for_saving['depth_x'] - \
                                                                 self.new_df_for_saving['depth_y']

                    self.new_df_for_saving['lat_difference'] = self.new_df_for_saving['lat_x'] - self.new_df_for_saving[
                        'lat_y']

                    self.new_df_for_saving['lon_difference'] = self.new_df_for_saving['lon_x'] - self.new_df_for_saving[
                        'lon_y']
    ...
    def saveResult_to_excel(self):
        ...
        if len(file_path) > 0:  # Если есть путь
            ...
            self.new_df_for_saving.to_excel(writer, 'Sheet1', index=False)
    ...
    для изменения названия колонки'''
    ...
    def building_graphics(self):
        ...
        # Строим график если выбран self.checkBox_lat_lon
        if self.checkBox_lat_lon_diff.isChecked() == True:
            self.companovka_for_lat = QtWidgets.QVBoxLayout(self.tab_lat_lon_diff)

            self.companovka_for_lon = QtWidgets.QVBoxLayout(self.tab_lat_lon_diff)

            self.mycanvas_lat = MyMpvCanvas(self, width=5, height=4, dpi=100)
            # создаем объект панель инструментов для работы с графиком

            self.mycanvas_lon = MyMpvCanvas(self, width=5, height=4, dpi=100)
            # В качесве аргумента передаем объект график

            self.canvastoolbar = NavigationToolbar(self.mycanvas_lat, self)

            self.canvastoolbar = NavigationToolbar(self.mycanvas_lon, self)
            # кладем  график объект в размещение

            self.companovka_for_lat.addWidget(self.mycanvas_lat)
            self.companovka_for_lon.addWidget(self.mycanvas_lon)

            # Кладем понель инструментов с графиком в размещение
            self.companovka_for_lat.addWidget(self.canvastoolbar)
            self.companovka_for_lon.addWidget(self.canvastoolbar)

            self.mycanvas_lat.axes.set_title("Разница координат catalog 1 & catalog2 ")
            # генерация номеров для xaxis
            # generated_numbers = list([i for i in range(0, len(self.new_df_for_saving['depth_difference']))])

            # self.mycanvas_depth.axes.set_xticks(generated_numbers)
            self.new_df_for_saving.plot(ax=self.mycanvas_lat.axes, kind='line', y='lat_difference',
                                        color='blue',
                                        marker='o')
            self.new_df_for_saving.plot(ax=self.mycanvas_lon.axes, kind='line', y='lon_difference',
                                        color='blue',
                                        marker='o')
